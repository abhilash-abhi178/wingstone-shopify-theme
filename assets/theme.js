...
window.Wingstone.formatMoney = (cents) => {
...
window.Wingstone.showToast = (message) => {
  const toast = document.querySelector('[data-toast]');
  const text = toast && toast.querySelector('[data-toast-text]');
...
const setRevealObservers = () => {
  const targets = document.querySelectorAll('[data-reveal], .js-reveal');
...
const setLenis = () => {
  ...
  window.Wingstone.lenis = new window.Lenis({ smoothWheel: true, lerp: 0.08, duration: 1.05 });

  const raf = (time) => {
    window.Wingstone.lenis.raf(time);
...
const setupProductFills = () => {
  document.querySelectorAll('[data-progress-fill]').forEach((bar) => {
    const progress = Number(bar.getAttribute('data-progress-fill') || 0);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hero && window.gsap) {
    gsap.from(hero.querySelectorAll('[data-animate-item]'), {
      y: 28,
      opacity: 0,
      duration: 0.9,
      stagger: 0.08,
